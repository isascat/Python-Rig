...
def build(
    ...
    headMainCtrl = control.Control( prefix = prefix + 'HeadMain', translateTo = neckJoints[-1], scale = rigScale * 25,
    ...
    headLocalCtrl = control.Control( prefix = prefix + 'HeadLocal', translateTo = headJnt, rotateTo = headJnt,
                                      scale = rigScale * 30, parent = headMainCtrl.C, shape = 'circleZ' )
    
    middleCtrl = control.Control( prefix = prefix + 'Middle', translateTo = neckCurveClusters[2], rotateTo = neckJoints[2],
                                  scale = rigScale * 25, parent = rigmodule.controlsGrp, shape = 'circleX' )
    ...
    mc.parent( neckCurveClusters[2:], headMainCtrl.C )
    mc.parent( neckCurveClusters[3], middleCtrl.C )
    mc.parent( neckCurveClusters[0:3], baseAttachGrp )
    ...
    neckIk = mc.ikHandle( n = prefix + '_ikh', sol = 'ikRPsolver', sj = neckJoints[0], ee = neckJoints[-1],
                           c = neckCurve, ccv = 0, parentCurve = 0 )[0]
    mc.poleVectorConstraint( neckloc,neckIk)  
    mc.hide( neckIk )
